...
_Unwind_Reason_Code fiber_unwind_stop(
    ...
    if ( actions & _UA_END_OF_STACK) {
        ...
        static intptr_t stop_token = -1;
        jump_fcontext( param, reinterpret_cast< void * >( stop_token) );
...
class fiber_context;
fiber_context unwind_fiber( fiber_context &&);
...
private:
    ...
    friend fiber_context unwind_fiber( fiber_context &&);
...
rep:
    ...
    fiber_context resume_from_any_thread_with( Fn && fn) && {
        ...
        if ( -1 == reinterpret_cast< intptr_t >( t.data) ) {
            return {};
        }
...
fiber_context unwind_fiber( fiber_context && other) {
    _Unwind_ForcedUnwind(
            new detail::foreign_unwind_ex{},
            detail::fiber_unwind_stop,
            std::exchange( other.fctx_, nullptr) );
    __builtin_unreachable();
}
