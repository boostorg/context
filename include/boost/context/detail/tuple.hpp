...
head_impl( std::tuple< S ... > && s,
    ...
    t = std::tuple< T ... >{ std::get< I >( std::forward< std::tuple< S ... > >( s) ) ... };
}
...
tail_impl( std::tuple< S ... > && s, index_sequence< I1 ... >,
    ...
    t = std::tuple< T ... >{ std::get< (Idx + I2) >( std::forward< std::tuple< S ... > >( s) ) ... };
}
...
public:
    ...
    void operator=( std::tuple< S ... > && s) {
        ...
        head_impl( std::forward< std::tuple< S ... > >( s),
...
public:
    ...
    void operator=( std::tuple< S ... > && s) {
        static_assert((sizeof...(T)) <= (sizeof...(S)), "invalid tuple size");
        tail_impl( std::forward< std::tuple< S ... > >( s), index_sequence_for< S ... >{},
                   t_, index_sequence_for< T ... >{} );
    }
};
